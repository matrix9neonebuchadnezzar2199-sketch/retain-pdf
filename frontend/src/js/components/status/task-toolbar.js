class StatusTaskToolbar extends HTMLElement {
  connectedCallback() {
    ...
    if (variant === "downloads") {
      ...
      this.innerHTML = `
        <button id="status-detail-btn" type="button" class="task-toolbar-btn task-toolbar-btn-compact secondary" aria-label="タスク詳細" title="タスク詳細">
        ...
        <a id="reader-btn" class="button-link secondary disabled task-toolbar-btn hidden" href="#" aria-label="対照閲覧" title="対照閲覧" aria-disabled="true">
          <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
        ...
        <a id="pdf-btn" class="button-link disabled task-toolbar-btn task-toolbar-btn-primary hidden" href="#" target="_blank" rel="noopener noreferrer" aria-label="PDF をダウンロード" title="PDF をダウンロード">
          <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
    ...
    this.innerHTML = `
      <button id="cancel-btn" type="button" class="task-toolbar-btn secondary" aria-label="タスクをキャンセル" title="タスクをキャンセル" disabled>
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M8.4 8.4l7.2 7.2M15.6 8.4l-7.2 7.2" stroke="currentColor" stroke-width="1.65" stroke-linecap="round"/>
        </svg>
        <span>キャンセル</span>
      </button>
      <button id="stop-btn" type="button" class="task-toolbar-btn secondary" aria-label="ポーリング停止" title="ポーリング停止">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <rect x="7.7" y="7.7" width="8.6" height="8.6" rx="2.2" fill="currentColor"/>
        </svg>
        <span>停止</span>
      </button>
      <button id="back-home-btn" type="button" class="task-toolbar-btn secondary hidden" aria-label="ホームに戻る" title="ホームに戻る">
        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
